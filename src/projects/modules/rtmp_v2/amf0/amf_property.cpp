...
namespace modules::rtmp
{
	...
	AmfProperty::AmfProperty(uint32_t number)
		: AmfProperty(static_cast<double>(number))
	{
	}
	...
	void AmfProperty::ToString(ov::String &description, size_t indent) const
	{
		...
		{
			case AmfTypeMarker::Number:
				description.AppendFormat("%.1f", GetNumber());
				break;

			case AmfTypeMarker::Boolean:
				description.AppendFormat("%s", GetBoolean() ? "true" : "false");
				break;

			case AmfTypeMarker::String:
				if (_string != nullptr)
				{
					description.AppendFormat("\"%s\"", GetString().CStr());
				}
				else
				{
		...
		if (show_type)
		{
			description.AppendFormat("  (%s)", EnumToString(GetType()));
		}
	}
