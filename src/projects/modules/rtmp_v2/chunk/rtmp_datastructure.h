...
{
	enum class MessageHeaderType : uint8_t
	{
		T0 = 0b00,
		T1 = 0b01,
		T2 = 0b10,
...
#pragma pack(push, 1)
	struct ChunkHeader
	{
		...
		{
			return basic_header_length + message_header_length + ((is_extended_timestamp) ? EXTENDED_TIMESTAMP_SIZE : 0U);
		}
		...
		static_assert(sizeof(ChunkHeader::extended_timestamp) == EXTENDED_TIMESTAMP_SIZE, "Extended timestamp size must be 4 bytes");
		...
		ov::String ToString() const
		{
			...
			if (basic_header_length > 0U)
			{
				...
				if ((basic_header.format_type == MessageHeaderType::T3) || (message_header_length > 0U))
				{
...
#pragma pack(pop)
	...
	{
	public:
		Message(const std::shared_ptr<const ChunkHeader> &header, const std::shared_ptr<ov::Data> &payload)
			: header(header),
			  payload(payload)
		{
		...
		std::shared_ptr<const ChunkHeader> header;
		...
		uint32_t ReadPayloadAsU32() const
		{
			...
			auto value = payload->GetDataAs<uint8_t>();
